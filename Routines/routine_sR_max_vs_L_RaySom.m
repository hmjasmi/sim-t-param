...
load routine_sR_max_vs_L_saved_channels
%%
P_tx = 1;
N0 = 1;
for i_loop = 1:length(L_range)
    ...
    while(iter<Iter)
        ...
        [~,opt_RaySom] = func_sR_MAX_GDA_SimplExact(H_RI,H_IT,Tl,N0,properties);
        r_ss_rs = r_ss_rs + func_compute_sR(eye(K,K),opt_RaySom.H_opt_ss,N0);
        iter = iter + 1;
    end
    R_ss_rs(i_loop) = r_ss_rs./Iter;
end
%%
figure
set(gcf, 'Units', 'Inches', 'Position', [0, 0, 2, 3.5], 'PaperUnits', 'Inches', 'PaperSize', [2, 3.5]);
